        ...
        public string GetAuthorizationURL(string[] scope, string redirectUrl = null)
        {
            ...
            if (scope != null && scope.Length > 0)
            {
                ...
                if (!String.IsNullOrEmpty(commaSeperatedScope))
					authURL.AppendFormat("&scope={0}", System.Uri.EscapeDataString(commaSeperatedScope));
            }

            if (redirectUrl != null && redirectUrl.Length > 0)
            {
				authURL.AppendFormat("&redirect_uri={0}", System.Uri.EscapeDataString(redirectUrl));
            }
