...
{
	public partial class AlertViewController : UIView
	{
		...
		private MenuViewController owner;

		private bool IsFilterAlertPresent;
		public AlertViewController ( nfloat w, nfloat h,  string title,  MenuViewController owner) {
			IsFilterAlertPresent = false;
			this.owner = owner;
			IsCategoryAlertOpen = true;
			_reports = new List<string> ();
			_reports.Add ("Current stock report");
			_reports.Add ("Sales report");
			height = h;
			width = w;
			Initialize(); 
		}

		public AlertViewController ( RectangleF bounds ) : base( bounds ) {
			Initialize();
		}
		...
		private void Initialize () {
			...
			var titleLabel=new UILabel() {Text = "Reports"};
			titleLabel.Font = UIFont.FromName ("Helvetica-Bold", 20f);
			//var messageLabel=new UILabel() {Text = "Select an role"};
			...
			string[] tableItems= _reports.ToArray();
		...
		public class TableSource : UITableViewSource
		{
			...
			private AlertViewController owner;
			public TableSource(string[] items, AlertViewController owner )
			{
				TableItems = items;
				this.owner = owner;
			}
			...
			{
				if (TableItems [indexPath.Row] == "Current stock report") {
					owner.CloseAlert();
					CurrentStockReportViewController mivc = owner.owner.Storyboard.InstantiateViewController("CurrentStockReportViewController") as CurrentStockReportViewController; 
					owner.owner.NavigationController.PushViewController(mivc,true);
				}

				if (TableItems [indexPath.Row] == "Sales report") {
					owner.CloseAlert(); 
					SalesReportViewController mivc = owner.owner.Storyboard.InstantiateViewController("SalesReportViewController") as SalesReportViewController; 
					owner.owner.NavigationController.PushViewController(mivc,true);
				}
