...
{
	public partial class RolesAlertViewController : UIView
	{
		...
		private UserDetailViewController owner;

		private bool IsFilterAlertPresent;
		public RolesAlertViewController 
		( nfloat w, nfloat h, List<string> _categories, string title,  UserDetailViewController owner) {
		...
		public RolesAlertViewController ( RectangleF bounds ) : base( bounds ) {
			Initialize();
		}
		...
		private void Initialize () {
			...
			var titleLabel=new UILabel() {Text = "Roles"};
			titleLabel.Font = UIFont.FromName ("Helvetica-Bold", 20f);
			var messageLabel=new UILabel() {Text = "Select a role"};
		...
		public class TableSource : UITableViewSource
		{
			...
			private RolesAlertViewController owner;
			public TableSource(string[] items, RolesAlertViewController owner )
			{
				TableItems = items;
				this.owner = owner;
			}
			...
			public override void RowSelected(UITableView tableView, NSIndexPath indexPath)
			{
				...
				owner.owner.UpdateCode(TableItems[indexPath.Row]);
				owner.CloseAlert();
