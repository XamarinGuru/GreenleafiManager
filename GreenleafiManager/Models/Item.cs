...
namespace GreenleafiManager
{
	...
	public class InventoryNS : NSObject
	{
		...
		public InventoryNS(Item inv, bool ignoreImages = false)
		{
			...
			ShopifyDescription = inv.ShopifyDescription;
			ShopifyTitle = inv.ShopifyTitle;
			ShopifyId = inv.ShopifyId;

			ShowOnWebsite = inv.ShowOnWebsite;
			LastCheckedDate = inv.LastCheckedDate.ToLocalTime().ToString("MM/dd/yyyy hh:mm tt");
			//LastCheckedDate = inv.LastCheckedDate;

			SetGLShortCode();
			SetSearchString();
			if (!ignoreImages)
			{
				if (inv.Images != null && inv.Images.Count > 0)
					Images = inv.Images;
				//else
				//	LoadImagesFromDrive();
			}
...
public string LastCheckedDate { get; set; }
		...
		[Export("ThumbnailImageUrl")]
		public UIImage ThumbnailImageUrl
		{
			get
			{
				var documentsDirectory = Environment.GetFolderPath(Environment.SpecialFolder.Personal);
				string jpgFilename = System.IO.Path.Combine(documentsDirectory, $"{Sku}.jpg"); // hardcoded filename, overwritten each time
				var localImage = System.IO.File.Exists(jpgFilename) ? UIImage.FromFile(jpgFilename) : null;
				if (localImage != null)
					return localImage;
				else
					return UIImage.FromFile("Images/no-thumb.png");
			}
		}
...
public void ReorderGridItems()
{
	_GridImages = _GridImages.OrderBy(x => x.SortOrder).ToList();
		...
		var data = AzureService.InventoryService.GetImagesFromAzure();
			if (data != null && data.Count > 0)
			{
				foreach (var ri in data)
				{
					...
					pi.position = ri.MainPic ? "1" : "-1";
					pi.ImageBytes = imageBytes;
					pi.OriginalId = ri.OriginalId;
					pi.PictureName = ri.PictureName;

					GridImage gi = new GridImage();
					gi.Image = Extensions.ToImage(imageBytes);
					gi.OriginalId = ri.OriginalId;
					gi.PictureName = ri.PictureName;
					var position=ri.MainPic ? "1" : "-1";
					gi.SortOrder = position == "-1" ? (_GridImages.Count + 1).ToString() : position;
					_GridImages.Add(gi);
				}
			}
		}
		...
		public Item ConvertToInventory()
		{
			...
			inv.ShowOnWebsite = ShowOnWebsite;
			inv.ShopifyTitle = ShopifyTitle;
			inv.ShopifyPrice = ShopifyPrice;
			inv.ShopifyDescription = ShopifyDescription;
		...
		public List<PhotoInfo> GetPhotoInfoList()
		{
			...
			if (Images != null)
			{
				...
				{
					var pi = new PhotoInfo();
					pi.ImagePath = img.LocalPath;
					pi.Position = img.position;
					pi.id = img.id;
					pi.product_id = img.product_id;
					pi.updated_at = img.updated_at.ToString();
					pi.created_at = img.created_at.ToString();
		...
		public void LoadImagesFromDrive()
		{
			if (Images == null || Images?.Count == 0)
			{
				...
				do
				{
					...
					if (localImage != null)
					{
						var prdctImage = new ProductImage()
						{
							id = Guid.NewGuid().ToString(),
							ImageBytes = Extensions.ToNSData(localImage),
							filename = String.Format("{0}{1}.jpg", Sku, (index > 0 ? "_" + index.ToString() : ""))
						};
						Images.Add(prdctImage);
						imageFound = true;
						if (AzureService.InventoryService.Items.Any(x => x.Sku == Sku))
							AzureService.InventoryService.Items.First(x => x.Sku == Sku).AddImageData();
					}
					else
					{
						imageFound = false;
